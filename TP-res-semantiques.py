class SemanticNetwork:
    ...
    def add_relation(self, subject, relation, obj, is_exception=False):
        if subject not in self.nodes: self.nodes[subject] = {}
        if relation not in self.nodes[subject]: self.nodes[subject][relation] = []
    ...
    def get_properties(self, node):
        ...
        def infer(current_node):
            if current_node not in self.nodes: return
...
net.add_relation('Autruche', 'vole', 'Non')

print("--- Partie 1 : Propagation (Canari -> respire) ---")
print(f"Le canari respire quoi ? {net.marker_propagation('Canari', 'respire')}")

print("\n--- Partie 2 & 3 : Héritage & Exceptions ---")
print(f"Propriétés du Canari : {net.get_properties('Canari')}")
print(f"Propriétés de l'Autruche : {net.get_properties('Autruche')}")